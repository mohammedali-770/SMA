 ...
 * on this helper alone. Coordinates are GeoJSON order: [longitude, latitude].
 */
...
/** Even-odd ray casting for a single linear ring ([lng,lat] pairs). */
...
/** Great-circle distance in km (Haversine); kept local so this module is self-contained. */
 ...
 * Sorted nearest-first by branch coordinates. UX pre-check only (server re-checks).
 */
