...
vi.mock('../../lib/staffAccessApi', () => ({ staffAccess: api }));
...
beforeEach(() => {
  vi.clearAllMocks();
  api.listStaff.mockResolvedValue([admin]);
  api.listAudit.mockResolvedValue([]);
  api.searchCandidates.mockResolvedValue([candidate]);
  api.setRole.mockResolvedValue({ id: candidate.id, role: 'accountant', changed: true });
});
afterEach(() => {
  cleanup();
  vi.restoreAllMocks();
...
describe('StaffAccessPanel', () => {
  ...
  it('searches existing accounts and refuses a role change without an audit reason', async () => {
    ...
    expect(api.searchCandidates).toHaveBeenCalledWith('Candidate', 20);
  ...
  it('applies an audited role change only after confirmation and refreshes the feeds', async () => {
    ...
    await waitFor(() => expect(api.setRole).toHaveBeenCalledWith('customer-1', 'accountant', 'Finance access'));
    await waitFor(() => expect(api.listStaff.mock.calls.length).toBeGreaterThanOrEqual(2));
    expect(api.searchCandidates.mock.calls.length).toBeGreaterThanOrEqual(2);
