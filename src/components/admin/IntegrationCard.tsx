  ...
  return (
    <div className="glass-card p-4 rounded-2xl bg-white/40 space-y-3.5">
      <div className="flex justify-between items-start border-b border-slate-100 pb-2.5">
        <div>
          <h4 className="text-xs font-black text-slate-800">{spec.title}</h4>
          <p className="text-[9.5px] text-slate-600 font-medium mt-0.5">{spec.subtitle}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {/* Provider select */}
        <div>
          <label className="block text-[9px] font-black text-slate-600 mb-1">Provider</label>
          <select
            value={providerName}
            disabled={disabled}
        ...
        {spec.publicFields.map(f => (
          <div key={f.key}>
            <label className="block text-[9px] font-black text-slate-600 mb-1">{f.label}</label>
            {f.type === 'bool' ? (
              <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-3 bg-slate-50/60 rounded-xl border border-slate-100">
        {spec.secretFields.map(f => (
          <div key={f.key}>
            <label className="text-[9px] font-black text-slate-600 mb-1 flex items-center gap-1">
        ...
        <p className="md:col-span-2 text-[8.5px] text-slate-600 font-medium flex items-start gap-1 leading-snug">
