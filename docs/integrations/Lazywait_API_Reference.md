...
| Field | Why not |
|---|---|
| ~~`subtotal`, `discount`, `tax`, `total`, `order_delivery_fee`~~ | **NOW SENT (2026-08-27, Q9 answered).** Ticket #3 printed `0.00` for subtotal/VAT/total on an order whose lines were right there at 23.00 and 5.00 — proving the POS **displays** what it is given rather than computing from the lines. All five are copied verbatim from the order snapshot (`orders.subtotal`, `vat_amount`, `total`, `delivery_fee`, and the three discount columns summed). `tax` is the VAT **contained within** `total`; line prices stay VAT-inclusive and unchanged. |
| `tax_percentage` | Still not sent. Optional, and the explicit `tax` amount says the same thing. A percentage is the one field that could invite the POS to recompute `subtotal × 1.15` and print 32.20 on a 28.00 order. |
| `is_paid` | Still not sent — the CLAUDE.md §6 financial signal reserved for a separate owner decision. What an order is *worth* is not whether it has been *settled*. |
