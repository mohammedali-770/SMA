...
half remains untested on both counts: no current Android build exists (see X2).
The 256 KB share-buffer limit is also untested against a real export, though the
only account large enough to hit it does not exist yet. |
