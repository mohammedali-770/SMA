...
| A6 | Data-subject rights: access, correction, deletion, **portability** | ⚠️ built 2026-09-08, needs the X2 build | Deletion and correction existed; **access and portability had no implementation at all** — confirmed live before building (zero `public` functions matching `%export%`, `%portab%`, `%my_data%`). `20260912120000_export_my_data.sql` adds `export_my_data()`: **zero arguments by design**, subject is always `auth.uid()`, so it cannot be pointed at another customer. Returns account, addresses, orders with line items, the full loyalty ledger and notification preferences; withholds the push **token** and all internal operational state. Surfaced at Profile → Account & privacy → *Get a copy of my data*, shared through React Native's core `Share` so it needs no new native module. **Not ✅ until the migration is applied and the X2 build ships the screen.** |
