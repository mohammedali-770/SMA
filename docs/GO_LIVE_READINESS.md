...
| # | Item | Note |
| --- | --- | --- |
| **D8** | **An ACTIVE Production Edge Function has no source in this repository** — `latency-probe`, version 2, `verify_jwt: true`, last updated 2026-08-30. Confirmed live: `list_edge_functions` returns it, and `supabase/functions/` has no such directory. It is therefore outside change control, outside the `Edge Function typecheck (Deno)` gate, and outside review. Either restore its source or delete the deployment; both are §5 actions. |
