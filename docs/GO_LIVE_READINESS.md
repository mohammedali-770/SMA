...
| A6 | Data-subject rights: access, correction, deletion, **portability** | ⚠️ APPLIED 2026-09-09; **the build is now ON a real device (iOS 1.0.0 (27), 2026-09-21) — but the export path itself has NOT been exercised by a human.** See the amendment at the end of this cell | Deletion and correction existed; **access and portability had no implementation at all** — confirmed live before building (zero `public` functions matching `%export%`, `%portab%`, `%my_data%`). `20260912120000_export_my_data.sql` adds `export_my_data()`: **zero arguments by design**, subject is always `auth.uid()`, so it cannot be pointed at another customer. Returns account, addresses, orders with line items **and their selected add-ons**, the full loyalty ledger, the comp flag, and notification preferences; withholds the push **token** and all internal operational state. The test applied to every field: *every number in the export must be accountable from the export itself* — which is why add-ons and the comp flag are in, since both sit inside figures the customer would otherwise be unable to reconcile. Surfaced at Profile → Account & privacy → *Get a copy of my data*, shared through React Native's core `Share` so it needs no new native module. **KNOWN LIMIT:** the share text crosses Android's Binder buffer (~1 MB, shared), so an export above 256 KB refuses with a support message rather than failing silently — a guard, not a cure. The real fix is a file attachment via `expo-file-system` + `expo-sharing`, which is a native dependency and therefore rides the next build. **APPLIED to Production 2026-09-09 07:32:09 UTC** (live version `20260909073209`, ledger row 86) on explicit owner approval naming the target by version. Verified live rather than inferred from a clean apply — which matters here, because a `plpgsql` body is not name-resolved at creation and this one reads **65 distinct columns across seven tables**: all 65 are present in live `information_schema` (0 missing), and the function was then called as a real customer inside a read-only transaction — which resolves every name at execution and is the stronger check. (This entry first said "38 columns across six tables"; review corrected it on #350.) It returns the 7 intended keys and **1 order out of the 71 in the table**, that one the caller's own; the anonymous path raises `42501` rather than returning an empty document that would look like a successful export of nothing; `anon` cannot execute it and `compute_order_snapshot` is still reachable by neither role. **Corrected 2026-09-16: the build exists.** iOS 1.0.0 (23) and the Android AAB were both built from `f82cecbe`, and `git grep -l export_my_data f82cecbe -- apps/mobile/` returns `services/api.ts` and `features/profile/dataExport.test.ts` — so the screen is **in both binaries**. What stands between this and ✅ is narrower than "the X2 build": the Android bundle is on Play's internal track, and the iOS build was **refused by TestFlight** over `NSMotionUsageDescription` (**B8**), so no tester on either platform has yet opened it. **Applied is not delivered, and built is not delivered either.** <br><br> **AMENDED 2026-09-21 — the DELIVERY half is settled for iOS, and the EXERCISED half is not.** iOS 1.0.0 (27) is installed and in daily use on a real device, and it descends from `f82cecbe`, so the *Get a copy of my data* screen is in the binary a tester is holding. That retires the blocker this row actually named: "no tester on either platform has yet opened it" is no longer true of the build. <br><br> **What is still unproven is the feature, not its delivery.** Nobody has pressed *Get a copy of my data*. (An earlier draft of this amendment said the build-27 session "exercised per-size closing and the category chips" — the per-size half of that is withdrawn: the live trail shows no size was closed that day. See `OWNER_ACTIONS.md` §40.) This repository's own standard is confirmation by USE, and by that standard `export_my_data()` has been proven only server-side — called as a real customer, returning 1 order out of 71 — never through the screen. **Pressing it once on build 27 closes this row for iOS.** The Android half remains untested on both counts: no current Android build exists (see X2). The 256 KB share-buffer limit is also untested against a real export, though the only account large enough to hit it does not exist yet. |
| A7 | Privacy notice in Arabic and English, matching actual behaviour | ⚠️ **re-marked 2026-09-10** — the review happened; two residuals | Legal documents are served from `legal_documents` (`src/lib/legal.ts`). Whether the text is current, bilingual and accurate is a content review. **That content review HAS been done and this row did not know it:** [`LEGAL_DOCUMENTS_AUDIT.md`](LEGAL_DOCUMENTS_AUDIT.md) records all nine rows read end to end on 2026-09-08 — four corrected to v2.1, five verified accurate — and `offers_loyalty_terms` went to v2.2 on 2026-09-09. All nine carry non-empty `content_ar`. **Two residuals keep it off ✅, and the second one changed on 2026-09-16:** the binding Arabic has never had a native read (`OWNER_ACTIONS.md` §29, §33a, §35) — and where **A9** used to sit, a live statement the software did not match, **A10** now sits: the privacy policy is accurate about the software and contradicts *itself* about its effective date. A9 is closed on the public page; A10 is not. |
...
| B3 | Privacy nutrition labels declared and accurate | ⚠️ **derived 2026-09-16; not yet entered in the console** | **All twelve Apple data types are now derived and tabled in [`APP_STORE_SUBMISSION.md`](APP_STORE_SUBMISSION.md) §4** — this row deliberately does not restate them, for the same reason C5 does not restate the Play form: two copies of a form Apple cross-checks is how one goes quietly wrong. **Derived, not copied** from `PLAY_STORE_SUBMISSION.md` §6 — Apple adds an axis Play has no equivalent of. **Used to Track You is NO for every type, and that is measured from the `.ipa`**: zero `ATTrackingManager`/`advertisingIdentifier`/`ASIdentifierManager` references across the app binary and all 15 frameworks, no `NSUserTrackingUsageDescription`, no `SKAdNetworkItems`, `NSPrivacyTracking: false`. **So no ATT prompt is required.** The one answer that is argued rather than read off a table: **Crash Data is *Linked to You* even though Sentry's own bundled manifest says `Linked: false`** — that describes the SDK's defaults, and `sanitize.ts:223-226` keeps `event.user.id`, so our reports carry the account id. The developer's declaration governs. Must agree with **C5**. |
| B4 | Privacy manifest / required-reason APIs | ✅ **measured from the build-23 `.ipa` on 2026-09-16** | This row said *"Verify Expo, Sentry and Maps SDKs ship manifests"* and nobody had. **14 `PrivacyInfo.xcprivacy` files ship.** The app's own declares `NSPrivacyTracking: false`, zero collected data types, and three required-reason API categories with valid reason codes — `FileTimestamp C617.1`, `UserDefaults CA92.1`, `SystemBootTime 35F9.1`. Sentry, ExpoApplication, ExpoConstants, ExpoDevice, ExpoNotifications, ExpoSystemUI, React-Core, React-cxxreact, React-timing, RNCAsyncStorage and boost/folly/glog all ship one. **A trap worth recording: most ship it as a sibling `*_privacy.bundle`, not inside the `.framework`**, so a naive look inside the framework directories reports a false absence. **Stated rather than glossed:** `ExpoLocation`, `ExpoImage`, `ExpoFileSystem`, `ExpoModulesCore` and the four `SDWebImage` frameworks ship none of their own; none is on Apple's commonly-used-SDK list today, and the app's own manifest covers the required-reason APIs it uses. Re-check if Apple extends that list. Detail: [`APP_STORE_SUBMISSION.md`](APP_STORE_SUBMISSION.md) §5. |
...
| C5 | Data safety form matches reality | ⚠️ **derived from the schema and the code; answered in Console 2026-09-16** | Was ⬜ and content-free while the whole form had already been derived. **All thirteen data types, each traced to the schema line or code path it comes from, are in [`PLAY_STORE_SUBMISSION.md`](PLAY_STORE_SUBMISSION.md) §6** — this row deliberately does not restate them, because two copies of a form Play cross-checks is how one of them goes quietly wrong. Three answers are argued there rather than guessed: **Name is optional**, not required; **crash logs and diagnostics are REQUIRED**, because the customer gets no opt-out; and the push token carries an **Advertising or marketing** purpose, because promos are opt-out (CLAUDE.md §7). **The "Shared" column is deliberately unanswered** — four vendors receive customer data and each is plausibly a *processor*, which Play excludes from "sharing"; that classification is yours and, where a DPA is unclear, counsel's. Under-disclosing is the dangerous direction. Must agree with **B3** (Apple's labels, the same facts in a different form), **It was blocked by A9 and no longer is** — as of 2026-09-16 the published policy names Google for the map and Apple for the iPhone reverse-geocode, and the public no-JavaScript page Play cross-checks serves that text. The form and the policy now agree. (`OWNER_ACTIONS.md` §34, closed.) **A10 does not re-block it**: that defect is the document's own effective date, not a processor claim, so nothing in the Data Safety form contradicts it. |
...
| **X2** | **No *current* production build exists for the code that would ship — FULLY REFUTED 2026-09-16. Both halves, not one.** **The iOS clause below was false when it was written, and it is retracted here rather than left standing.** It said *"the newest iOS production build is 1.0.0 (22), commit `6265781a`, 2026-08-26 — ~54 commits stale"*. The newest iOS production build is **1.0.0 (23)**, `fcb7d682-…`, commit **`f82cecbe`**, FINISHED **2026-09-14** — `STORE` distribution, `production` profile, built by the GitHub App **seven minutes after the Android AAB, from the same commit**. It is functionally current on the same test the Android half used: `git diff --name-only f82cecbe HEAD -- apps/mobile/` returns nothing.

**HOW A ROW CORRECTED TODAY WAS WRONG TODAY:** its Android half was re-read off the Play Console on 2026-09-15/16 while its iOS half was left on the **2026-09-03** EAS read its own evidence column cites. **Half a row refreshed is a row that lies about the other half**, and the lie is harder to catch than a wholly stale row, because the dateline says today. Caught by running `eas build:list --platform ios` rather than trusting the cell.

**What is genuinely open on iOS is a purpose string and a rebuild, not staleness.** Build 23 could not be uploaded to TestFlight — App Store Connect error `90683`, missing `NSMotionUsageDescription`, caused by this repository's own B8 change. Fixed 2026-09-16; a build carrying the fix is what remains. See **B8** and [`APP_STORE_SUBMISSION.md`](APP_STORE_SUBMISSION.md).

*The original text follows, retracted:* ~~The original reading stands for iOS: the newest **iOS** production build is 1.0.0 (22), commit `6265781a`, 2026-08-26 — **~54 commits stale**, predating the delivery-to-POS go-live.~~ **Both Android clauses are now false and are retracted here rather than left standing.** An Android production app bundle **exists** — `v1.0.0`, versionCode 2, commit `f82cecbe`, `targetSdkVersion 36` — and has been **uploaded to Play's internal-testing track**, which is Active. And `apps/mobile/eas.json` **does** carry `submit.production.android`; what it lacks is a service-account key, which is a **credential, not a missing track** — a distinction this row collapsed. **The bundle is also still functionally current:** `git diff --name-only f82cecbe HEAD -- apps/mobile/` returns nothing, so every mobile change on the default branch is in it. ~~What survives is iOS, and it survives whole.~~ **Nothing survives of the staleness claim on either platform.** | Android half re-read off the AAB's own manifest (C1) and the Play Console on 2026-09-15/16; **iOS half re-read live from the EAS CLI on 2026-09-16 and retracted** — the 2026-09-03 reading it had been carrying is what made it wrong | **iOS:** a build carrying the `NSMotionUsageDescription` fix (B8), then TestFlight, then the §1 physical-device gate. **Android:** the record, the bundle and the internal track all exist — what is open is **C8**'s closed-testing gate (calendar time), the remaining listing items (**G4**), and step 5 of `OWNER_ACTIONS.md` §38. The Android half is no longer measured in build time. |
...
| --- | --- | --- | --- |
| **A9** | ~~**The privacy policy names the wrong map sub-processor**~~ — **CLOSED 2026-09-16** | ✅ **corrected in the database AND confirmed on the public page after the rebuild** | `privacy_policy` v2.1 says `Mapbox — the map you use to choose a delivery location` in **both languages**. The shipped web bundle carries a Google Maps key (`AIza` present) and **no Mapbox token** (`pk.` absent, 0 occurrences), and `googleMaps.ts` uses Google **Places** for address search. A named processor receives nothing while an unnamed one receives the customer's **delivery coordinates** — wrong in both directions, in the document destined for both store listings. Replacement text, evidence and its limits: [`legal/PRIVACY_POLICY_MAP_PROCESSOR_CORRECTION.md`](legal/PRIVACY_POLICY_MAP_PROCESSOR_CORRECTION.md); owner action `OWNER_ACTIONS.md` §34. **That variable was read, and the row that said it could not be is corrected.** `eas env:list --environment production` gives `EXPO_PUBLIC_MAP_PROVIDER=google`, with **zero Mapbox variables** in that environment — so the Mapbox branch has no token and could not work even if selected. Native and web agree, which settled §34 on **Option A (Google only)**. **`privacy_policy` v2.3 is live as of 2026-09-16**, both languages. It names Google for the map and **Apple for the iPhone reverse-geocode** — a sub-processor no version had ever disclosed. v2.2, half an hour earlier, said "the map **and address search**", promising Google received something customers cannot send it: there is no address search in either customer channel, only in the admin console. Each edit changed exactly one line, with every other byte verified identical.

**IT IS ✅ ONLY BECAUSE THE SECOND STEP RAN, AND THAT IS THE MOST USEFUL THING ON THIS ROW.** Publishing the row corrected the app and every JavaScript-enabled reader immediately. It did **not** correct the **no-JavaScript snapshot**, because B7's prerender plugin runs with `apply: 'build'` and bakes the documents into `legal.html` during `vite build` — measured on the deployed page immediately after publishing: still `v2.1 · 2026-09-08`, still `Mapbox — the map you use to choose a delivery location.`, byte-identical response size. **A store policy checker is exactly a no-JavaScript reader**, so at that moment nothing had changed from Google's or Apple's point of view.
...
| **B8** | **The iOS binary declared three purpose strings for capabilities the app never uses** — ⚠️ **TWO of the three were right; the third broke the upload and is corrected 2026-09-16** | `expo-location`'s config plugin is auto-applied and injected `NSLocationAlwaysAndWhenInUseUsageDescription`, `NSLocationAlwaysUsageDescription` and `NSMotionUsageDescription`, all generic `Allow $(PRODUCT_NAME) to…` placeholders. An Always-location and a Motion declaration invite an App Review question with no good answer, since neither capability is used. Fixed by declaring the plugin explicitly with `false` for those three, which makes `applyPermissions` delete them. **Verified from the resolved config, not from source**: `expo config --type introspect` now yields exactly **one** usage description — the corrected when-in-use string — and no `UIBackgroundModes`.

**THAT SENTENCE IS THE DEFECT, AND IT READS LIKE THE EVIDENCE.** On 2026-09-16 App Store Connect **refused the TestFlight upload** of build 23 — `90683: Missing purpose string in Info.plist` / `EAS_UPLOAD_TO_ASC_MISSING_PURPOSE_STRING`, naming **`NSMotionUsageDescription`**. Apple's own text states the rule: *"If you're using external libraries or SDKs, they may reference APIs that require a purpose string. **While your app might not use these APIs, a purpose string is still required.**"* Measured against the artifact rather than argued: `ExpoLocation.framework` inside the `.ipa` carries **54** references to `CMMotionActivity`/`CoreMotion`. **Apple requires the string on LINKAGE, not on use**, so "neither capability is used" was true of our code and irrelevant to the validator.

**The two Always-location props were right and stay `false`** — the rejection named the motion key and nothing else.

**The lesson is why this row is ⚠️ and not ✅ again.** `expo config --type introspect` reads the resolved **config**. It proved the key was gone; it could not prove the key was **unnecessary**. Only the binary and Apple's validator can, and no iOS submission was attempted in the six days between this change and the next build, so nothing could have caught it. **A config check is not an artifact check** — the same shape as reading the AAB manifest for C1, and as fetching the deployed legal page rather than the database row (A9). Fixed with an honest string in `app.json` and **both** locale files, and pinned by `apps/mobile/src/components/iosPurposeStrings.test.ts` — 9 assertions, mutation-tested three ways, all killed. It returns to ✅ when a build carrying the string uploads. Full record: [`APP_STORE_SUBMISSION.md`](APP_STORE_SUBMISSION.md) §3. |
