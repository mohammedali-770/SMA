...
| **X6** | ~~The `ready` push tells delivery customers to come and collect.~~ **FIXED IN SOURCE 2026-09-03.** `order_type` now rides the order row `push-dispatch` already reads, and `ready` — the only status whose meaning differs by fulfilment — branches on it. Pickup copy is byte-identical; delivery gets *"ready and will be on its way shortly"*, which promises no time and does not pre-empt `out_for_delivery`. Pinned by `pushReadyCopyWiring.test.ts`, mutation-tested against four regressions. | source-verified | **Needs a `push-dispatch` redeploy (v6)** — a §5 action, recorded as `OWNER_ACTIONS.md` §26, where the redeploy is also shown to carry no behavioural drift. **Sequence it before X5:** the lifecycle rehearsal sends this copy to a real customer. The Arabic is engineering-drafted and wants a native read first. |
...
8. **X5 — the order lifecycle past `received` has never run in Production.** **X6,
   the `ready` push telling delivery customers to collect, is fixed in source** and
   needs a `push-dispatch` redeploy (§26). Deploy that **before** the rehearsal, or
   the rehearsal sends the wrong message to a real person. *(2026-09-03)*
