...
| G4 | Store listing assets | ⬜ **inventoried 2026-09-15 — two of six now done** | Measured rather than listed: an exhaustive image inventory of the tree returns 11 files and **none** was a store asset. **DONE — the 512×512 Play icon and the 1024×500 feature graphic**, both generated from the approved masters and guarded in CI. The icon: Play takes an upload rather than reading the AAB, and every icon here was 1024×1024 (the largest inside the bundle is a 432×432 adaptive foreground), so the listing could not be saved. `scripts/build-play-icon.mjs` now generates `assets/store/play-icon-512.png` from the same master, and `npm run play-icon:check` guards it in CI beside `logo:check`. The feature graphic is `assets/store/play-feature-graphic-1024x500.png` — the brand mark on warm off-white `#F5F1E8` at 76% of canvas height, fully opaque as Play requires, **deliberately wordless** because Play overlays the app name on some surfaces and baked-in text would need one graphic per listing language. It is compliant rather than designed; swapping it needs no new release, since the feature graphic is not tied to a build. **Still missing: phone screenshots** (min 2 — and capture is itself blocked behind X1, since signing in needs a Saudi number or the test-OTP entry), **short description** (≤80 chars) and **full description** (≤4000) in both languages, and the **app category** (Food & Drink, a Console-only field). **Do not reuse existing copy for the descriptions:** `metadata.json` describes the monorepo and `index.html`'s title advertises the POS, neither of which a customer sees; and the marketing site claims **16 branches** where Production has **1 active**. Contact details are ready — `info@spicymeal.com.sa` / `+966920031495`, live in `app_settings`. |
