...
| 73 | 20260827140000 | product_images_bucket | ? | 20260827195223 | product_images_bucket | ? | ? | ✔ present live | **UNRECORDED AT APPLY TIME** | none | high if `db push` | **GAP ROW, written 2026-08-28 from a live read, not from a record of the apply.** This migration is applied — `select version, name from supabase_migrations.schema_migrations` returns `20260827195223 / product_images_bucket` — but no ledger row was written when it was applied, and CLAUDE.md §8's file table does not list it either. **What is recorded here is only what a live read proves: that it is applied, and under which version.** The `repo skel`, `live skel` and `class` columns are therefore `?`, not `=`/`B`: those columns assert repository/live CONTENT equivalence under §4's fingerprint transform, and no such comparison was made. Marking them `=`/`B` would tell a drift analysis that two unverified bodies are confirmed matches — the precise error this row exists to avoid. Who approved it, at what UTC time, by which mechanism, against which merged commit, and with what post-apply verification are all **unknown to the session writing this row**, and are deliberately left blank rather than guessed. Closing this properly needs whoever performed it. Recorded so the ledger does not jump 72 → 75 in silence, which would read as though nothing happened between them |
| 74 | 20260827150000 | menu_display_order | ? | 20260827195309 | menu_display_order | ? | ? | ✔ present live | **UNRECORDED AT APPLY TIME** | none | high if `db push` | **GAP ROW, same provenance and same limits as row 73, including `?` rather than `=`/`B` for the two fingerprint columns and the class** — applied 46 seconds after it (`20260827195309`), unrecorded at the time, reconstructed 2026-08-28 from a live read alone. Approval, timing, mechanism and verification are unknown to the session writing this row and are left blank rather than invented |
...
feature is **applied but not yet live**: the migration defines the RPCs, and the
two Edge Functions that call them have not been deployed, so the login path it
exists to protect is still unrate-limited. Class B again, so the totals move to
**77/78** above.

Rows **73–75** were all written on 2026-08-28, and only one of them was written
by the session that performed the application. **73 and 74 are GAP ROWS** —
`product_images_bucket` and `menu_display_order` were applied on 2026-08-27 and
never recorded, which is why this table jumped from 72 straight to a
2026-08-28 application until now. They carry only what a live read proves; their
approval, timing, mechanism and post-apply verification are blank because they
are unknown, not because they were omitted for brevity. **75** is
