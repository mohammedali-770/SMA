...
(§28), `branch_availability_retention` (§30), and the `noop` probe. Rows 57–85
...
columns across six tables, so one renamed column would have shipped a data-rights
endpoint that raises for every customer who presses the button — with an apply
output indistinguishable from a good one. The columns were checked against
`information_schema` before sending and the function was **called** afterwards.
