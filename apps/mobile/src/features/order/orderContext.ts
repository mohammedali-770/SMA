...
/**
 * Launch/menu gate decision: true → the customer must be sent to the blocking
 * /select screen. Only fires once the persisted context has been hydrated and
 * the catalog finished loading without error, so the gate can never bounce the
 * customer during startup, on a slow network, or on a load failure (those show
 * their own states). A missing OR no-longer-valid context both force selection.
 */
export function shouldForceSelection(s: {
  ready: boolean;        // persisted context hydrated
  loading: boolean;      // catalog still loading
  error: string | null;  // catalog load failed (ErrorView owns the screen)
  valid: boolean;        // context valid against the live catalog
}): boolean {
  return s.ready && !s.loading && !s.error && !s.valid;
}
