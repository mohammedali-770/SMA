...
export function HomeMenuScreen({ suppressInvalidRedirect = false }: { suppressInvalidRedirect?: boolean } = {}) {
  ...
  return <View style={styles.root}>
    <View style={[styles.topBar, { paddingTop: insets.top + space.s2 }]}>
      <View style={styles.brandRow}><Image source={require('../../../assets/icon.png')} style={styles.mark} contentFit="cover" /><View><Text variant="title">{pick('Spicy Meal', 'سبايسي ميل')}</Text><Text variant="caption" tone="secondary" numberOfLines={1}>{pick('Since 1997', 'منذ 1997')}</Text></View></View>
...
const useStyles = makeStyles((color) => ({
  root: { flex: 1, backgroundColor: color.appBg }, topBar: { flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'space-between' as const, paddingHorizontal: space.s4, paddingBottom: space.s3, backgroundColor: color.appSurface, borderBottomWidth: 1, borderBottomColor: color.appLine }, brandRow: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: space.s2 }, mark: { width: 38, height: 38, borderRadius: radius.sm }, langBtn: { paddingHorizontal: space.s3, paddingVertical: 6, borderRadius: radius.pill, borderWidth: 1.5, borderColor: color.ember },
