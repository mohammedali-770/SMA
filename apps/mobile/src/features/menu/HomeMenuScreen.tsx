...
import {
  Pressable, ScrollView, SectionList, TextInput, View,
...
export function HomeMenuScreen() {
  ...
  useEffect(() => {
    if (shouldForceSelection({ ready: orderCtx.ready, loading, error, valid: orderCtx.valid })) router.replace('/select');
  ...
  const renderItem = useCallback(
    ({ item }: { item: MenuSectionItem }) => <ProductCard product={item.product} hasModifiers={item.hasModifiers} onAdd={handleAdd} />,
    [handleAdd],
  );
        ...
        <Pressable style={[styles.branchRow, rtlRow]} onPress={() => router.push('/select')} accessibilityRole="button">
          ...
          <View style={{ flex: 1 }}>
            <View style={[styles.ctxTopRow, rtlRow]}>
              <StatusPill label={orderCtx.context.orderType === 'pickup' ? t('otPickup') : t('otDelivery')} tone="info" />
              {selectedBranch ? <OpenClosedBadge open={branchOpen} /> : null}
            </View>
            <Text variant="heading" numberOfLines={1}>{pick(orderCtx.context.branchNameEn, orderCtx.context.branchNameAr)}</Text>
          ...
          {sections.length === 0 ? (
            ...
            <SectionList
              ...
              renderSectionHeader={({ section }: { section: SectionListData<MenuSectionItem, MenuSection> }) => (
                <Text variant="title" style={styles.sectionTitle}>{pick(section.category.nameEn, section.category.nameAr)}</Text>
              )}
...
function MenuSkeleton() {
  const skeleton = useSkeletonStyles();
  return (
...
const useSkeletonStyles = makeStyles((color) => ({
  ...
  card: { flexDirection: 'row' as const, backgroundColor: color.appSurface, borderRadius: radius.lg, overflow: 'hidden' as const, borderWidth: 1, borderColor: color.appLine },
...
const useStyles = makeStyles((color) => ({
  ...
  langBtn: { paddingHorizontal: space.s3, paddingVertical: 6, borderRadius: radius.pill, borderWidth: 1.5, borderColor: color.ember },
  branchRow: {
    flexDirection: 'row' as const, alignItems: 'center' as const, gap: space.s3,
    marginHorizontal: space.s4, marginTop: space.s3, padding: space.s3,
    backgroundColor: color.appSurface, borderRadius: radius.md,
    borderWidth: 1, borderColor: color.appLine, overflow: 'hidden' as const,
  },
  ...
  changeBtn: { paddingHorizontal: space.s3, paddingVertical: space.s2, borderRadius: radius.pill, borderWidth: 1.5, borderColor: color.ember },
  closedNotice: {
  ...
  searchWrap: {
    flexDirection: 'row' as const, alignItems: 'center' as const, gap: space.s2,
  ...
  cartCount: {
    ...
    backgroundColor: color.emberDeep, alignItems: 'center' as const, justifyContent: 'center' as const,
  },
}));
