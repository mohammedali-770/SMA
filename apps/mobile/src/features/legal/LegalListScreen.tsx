...
export function LegalListScreen() {
  ...
  return <Screen background={colors.appBg} edges={['top','left','right']}><Header title={t('legalSupport')} showBack />
    {error ? <ErrorView message={pick('Could not load documents.','تعذّر تحميل المستندات.')} onRetry={load} retryLabel={t('retry')} /> : docs===null ? <LoadingView label={t('loading')} /> :
