...
**A naive bulk apply would still sweep the frozen Moyasar file in**, because `20260824100000` sorts ahead of everything applied on 2026-08-25. Any future `supabase migration` operation must name its target explicitly. That is now the *only* thing a bulk apply could sweep in, which makes it more dangerous rather than less: with one outstanding file, "apply the outstanding migrations" reads like a no-op and is in fact the one instruction that would break the §6 freeze. Any `supabase migration` operation must still name its target explicitly.
