...
**Two repository files are unapplied, for different reasons.**
...
**A naive bulk apply would still sweep the frozen Moyasar file in**, because `20260824100000` sorts ahead of everything applied on 2026-08-25. Any future `supabase migration` operation must name its target explicitly. The point now has teeth it did not have before: with two unapplied files sitting side by side, one of which is deliberately frozen, "apply the outstanding migrations" is no longer a safe instruction to follow literally.
