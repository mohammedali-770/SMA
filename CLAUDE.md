...
**The deploy-order trap is now the other way round.** `lazywait-sync` reads `order_items.variant_id` and a `product_variants` embed through `ORDER_ITEM_SELECT`. Redeploying it *before* `…120000` was applied would have made PostgREST reject the select, and the handler does not check that error, so every order would have been blocked from the kitchen under a misleading `no_items` reason. **That risk is closed:** the migrations are applied, so redeploying `lazywait-sync` is now safe and is the remaining step, not a hazard.
