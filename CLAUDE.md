...
call. `export_my_data()` reads 38 columns across six tables, so one renamed
column would have shipped a data-rights endpoint that fails for every customer
who presses the button, with apply output indistinguishable from a good one. All
38 were checked against `information_schema` before sending, and the function was
**called** afterwards. A `language sql` body is the contrast: PostgreSQL
validates it at creation and the same mistake aborts loudly.
