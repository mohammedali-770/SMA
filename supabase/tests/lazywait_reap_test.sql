...
begin
  ...
  insert into public.orders
    (id, order_number, branch_id, order_type, subtotal, total,
     ...
     pos_sync_started_at, pos_sync_deadline_at, sync_attempt_count, updated_at) values
    (v_young,  'R-1', gen_random_uuid(), 'pickup', 10, 10, 'syncing', null,       null,      now()-interval '1m', now()+interval '9m', 0, now()-interval '2 minutes'),
    (v_synced, 'R-2', gen_random_uuid(), 'pickup', 10, 10, 'synced',  'REF_DONE', null,      now()-interval '5m', now()+interval '5m', 0, now()-interval '30 minutes'),
    (v_withref,'R-3', gen_random_uuid(), 'pickup', 10, 10, 'syncing', 'REF_LOST', now()-interval '25m', now()-interval '5m', now()+interval '5m', 2, now()-interval '30 minutes'),
    (v_amb,    'R-4', gen_random_uuid(), 'pickup', 10, 10, 'syncing', null,       now()-interval '25m', now()-interval '5m', now()+interval '5m', 1, now()-interval '30 minutes'),
    (v_safe,   'R-5', gen_random_uuid(), 'pickup', 10, 10, 'syncing', null,       null,      now()-interval '2m', now()+interval '8m', 1, now()-interval '30 minutes'),
    (v_maxed,  'R-6', gen_random_uuid(), 'pickup', 10, 10, 'syncing', null,       null,      now()-interval '5m', now()+interval '5m', 4, now()-interval '30 minutes');
  ...
  -- Summary counts: 1 recovered, 1 confirmation_required, 1 requeued, 1 dead-lettered.
  ...
  -- Idempotency: a second reap leaves no stale 'syncing' rows to touch.
  v_res := public.reap_stale_lazywait_syncs(10, 5);
  ...
  if (select count(*) from public.integration_sync_logs
        where provider = 'lazywait'
          and error in ('recovered_stale_syncing_with_ref',
                        ...
                        'stale_syncing_no_ref_dead_letter')) <> 4 then
    raise exception 'LOGGING FAILED: expected 4 recovery log rows';
  end if;
