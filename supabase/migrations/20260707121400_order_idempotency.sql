-- ============================================================================
-- Spicy Meal — order idempotency. A flaky mobile network can make a client
-- retry a checkout that actually succeeded, creating a DUPLICATE order (real
-- money/food loss at a restaurant). place_order gains an optional idempotency
-- key: a retry with the same key returns the already-created order.
--
-- place_order stays the single authoritative order path; this only adds the
-- 8th (defaulted) argument, so existing callers that pass no key behave exactly
-- as before.
-- ============================================================================

alter table public.orders add column if not exists idempotency_key uuid;
-- One order per (customer, key). Partial: only enforced when a key is supplied.
create unique index if not exists orders_idempotency_idx
  on public.orders (customer_id, idempotency_key)
  where idempotency_key is not null;

-- Replace the 7-arg place_order with the 8-arg (idempotency-aware) version.
