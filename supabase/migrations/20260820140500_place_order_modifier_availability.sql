-- ============================================================================
-- Spicy Meal — place_order: per-branch modifier availability + lazy expiry
--
-- THE ONLY MIGRATION IN THE BRANCH-OPERATIONS WORK THAT TOUCHES THE ORDER PATH.
-- Isolated deliberately so it can be reviewed and reverted on its own; every
-- other phase leaves place_order alone.
--
-- The body below is the CURRENT function copied VERBATIM from
-- 20260710120100_place_order_delivery_zone.sql — extracted programmatically,
-- not retyped — with exactly two additions:
--
--   1. The existing product-availability guard gains
--      `and (bpa.snoozed_until is null or bpa.snoozed_until > now())`, so a
--      snooze whose timer has passed stops blocking immediately instead of
--      waiting for the sweeper's next tick. An untimed closure still blocks.
--
--   2. The existing modifier loop gains the same check against
--      branch_modifier_availability. Nothing else enforces modifier stock: the
--      deferred contract trigger (20260810132000) validates cardinality, not
--      availability, and returns early for anything that is not a cash order.
--
-- Nothing else changes. Pricing, VAT extraction, coupons, loyalty, the
-- delivery-zone checks and the two-pass persistence are byte-identical to the
-- function this replaces.
--
-- compute_order_snapshot IS NOT TOUCHED. It carries the twin of the product
-- guard for the online-payment path and is payment-adjacent code under the
-- CLAUDE.md §6 freeze. The consequence is explicit and worth stating plainly:
-- modifier availability is enforced for cash orders and NOT for online checkout
-- sessions until that freeze lifts. Online payment is not the live path today
-- (the provider is unselected and automatic refunds are disabled), which is what
-- makes the gap acceptable rather than merely convenient.
--
-- NO GRANTS ARE EMITTED HERE, DELIBERATELY. `create or replace function`
-- preserves the existing ACL, and 20260724200000_order_read_contracts.sql
-- REVOKED execute on place_order from `authenticated` and wrapped it in the
-- customer-safe place_customer_order. Re-granting here — the obvious thing to
-- copy from the 20260710120100 original — would silently undo that hardening and
-- hand customers the raw function back.
-- ============================================================================
