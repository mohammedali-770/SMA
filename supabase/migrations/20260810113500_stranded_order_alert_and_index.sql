...
-- Cover exactly the terminal/non-retrying set read by
-- order_integrity_health_summary(). INCLUDE lets the aggregate filters/counts be
-- satisfied from the index without revisiting each order heap row when visibility
-- permits an index-only scan.
...
-- Preserve the existing deterministic derive implementation under a private-ish
-- service-role name, then wrap it. The guarded rename makes this migration safe
-- to re-run in a disposable migration-chain test database.
do $rename$
begin
