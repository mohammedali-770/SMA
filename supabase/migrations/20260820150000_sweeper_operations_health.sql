-- Operations Health: put `branch-availability-sweep` on the board.
--
-- THE GAP
-- 20260820111000 scheduled `branch-availability-sweep` (every minute) but did
-- not register it in `operations_health_snapshot_internal`. The health query is
-- `from expected e left join cron.job j`, so `expected` drives it: a job absent
-- from the allowlist produces no card, no state and no alert. It is invisible
-- rather than alarming — which is why deferring registration was safe, and why
-- it now has to be done deliberately rather than never.
--
-- The consequence of leaving it out is quiet and bad: if the sweeper dies,
-- every timed item closure, option closure and delivery pause stops reopening.
-- Nobody would learn that from the console; they would learn it from a branch
-- asking why yesterday's item is still off.
--
-- WHAT THIS CHANGES
-- One row in the allowlist, its matching row in the exception-path fallback
-- array, and the two counts that describe the allowlist:
--     expected_jobs  5 -> 6
--     automation_jobs 2 -> 3
--     critical_jobs   3 -> 3   (unchanged)
--
-- NON-CRITICAL, DELIBERATELY
-- The sweeper only ever REOPENS things. If it stops, closures outlive their
-- timers — over-blocking, never over-selling. That is fail-safe, so it belongs
-- with the optional automation crons whose rollup is surfaced as a warning and
-- never touches `database_jobs.state` or the overall platform state. The three
-- critical application crons keep the platform rollup entirely to themselves,
-- exactly as before.
--
-- Its staleness window is 6 minutes, the same as the other every-minute jobs.
--
-- SAFETY
-- - The body below is the 20260807150000 body VERBATIM plus the additions
--   above. It was produced by programmatic extraction and insertion, then
--   diffed against the original: 16 lines added, 5 pre-existing lines changed
--   in place (two comment lines, the digest row gaining a trailing comma, and
--   the two counts). Nothing else differs.
-- - No table, column, constraint, policy, grant or cron job is touched. This
--   migration reads pg_cron; it does not schedule, unschedule or alter anything.
-- - `create or replace` only, so it is idempotent and the existing ACL is
--   preserved. The grants are re-emitted anyway because they are unchanged and
--   the file should be readable on its own.
-- - The alert engine needs no change: per-job conditions are emitted under the
--   hardcoded subsystem `database_jobs` with the fingerprint
--   `database_jobs:job_health:branch-availability-sweep`, and severity follows
--   the job's own `critical` flag — so this job derives WARNING conditions, and
--   `database_jobs` already has a name in both language arms of
--   `operations_alerts_render_event`.
--
...
comment on function public.operations_health_snapshot_internal() is
  'Read-only Operations Health snapshot. Since 20260807 it carries an order_flow card in the critical set; since 20260820 the allowlist also observes branch-availability-sweep as a NON-critical automation cron (6-minute staleness), so a dead sweeper is visible as a warning without ever flipping the platform rollup. Returns no PII, secrets, raw provider payloads, tokens, cron commands or external provider probe results.';

-- ---------------------------------------------------------------------------
-- Rollback
-- ---------------------------------------------------------------------------
-- Re-apply the operations_health_snapshot_internal body from
-- 20260807150000_order_flow_health_card.sql. That removes the sweeper from the
-- allowlist, at which point it becomes invisible again rather than failing.
